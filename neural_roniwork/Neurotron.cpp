...
void Neurotron::evaluate(MatrixXd input, MatrixXd weight, MatrixXd target_out, MatrixXd target_class, MatrixXd bias) {
	...
	m_class_match_count = count_matches(m_output, target_class);
}
...
const int rw_convert_to_class(MatrixXd output) {
	int c = 0;
	for (int i = 2; i >= 0; i--) {
...
const int count_matches(MatrixXd output, MatrixXd target) {
	...
	for (int i = row_count - 1; i >= 0; i--) {
		class_err_count += (int)(rw_convert_to_class(output.row(i)) != target(i, 0));
	}
